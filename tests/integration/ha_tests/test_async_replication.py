...
async def test_deploy_async_replication_setup(ops_test: OpsTest, first_model, second_model) -> None:
...
@pytest.mark.abort_on_fail
async def test_async_replication(ops_test: OpsTest, controller, first_model, second_model) -> None:
    ...
    async with ops_test.fast_forward():  # replace with second model fast-forward.
    ...
    run_action = await leader_unit.run_action("promote")
    ...
    await check_writes(ops_test, extra_model=(await second_model))


async def test_async_replication_failover_in_main_cluster(ops_test: OpsTest, first_model, second_model) -> None:
...
async def test_async_replication_failover_in_secondary_cluster(ops_test: OpsTest, first_model, second_model) -> None:
